...
export const SqlOutput: React.FC<SqlOutputProps> = ({
  ...
  // Basic syntax highlighter helper for SQL / GraphQL
  const renderHighlightedCode = (text: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      return (
        <div key={idx} className="flex leading-6 font-mono text-xs hover:bg-slate-800/30 px-2 rounded">
    ...
    <div className="flex flex-col h-full rounded-2xl bg-[#090d18] border border-slate-800 shadow-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-slate-900/90 border-b border-slate-800/80 backdrop-blur-md">
        ...
        <div className="flex items-center space-x-2">
          <span className="hidden sm:inline-block font-mono text-[11px] text-slate-400 bg-slate-950/80 px-2 py-1 rounded border border-slate-800">
          ...
          <button
            onClick={handleCopy}
            className={cn(
              "flex items-center space-x-1.5 px-3 py-1 rounded-lg text-xs font-medium border transition-all duration-200",
              copied
      ...
      {/* Code Editor Body */}
      <div className="flex-1 p-3 overflow-auto bg-[#070a12] relative font-mono select-text">
        {isGenerating ? (
      ...
      <div className="px-4 py-2 bg-slate-900/90 border-t border-slate-800/80 flex flex-wrap items-center justify-between text-[11px] text-slate-400">
        ...
        <div className="flex items-center space-x-2 text-[10px] text-slate-500 font-mono">
          ...
          <span>•</span>
          <span className="text-emerald-400">Zero Injection Detected</span>
        </div>
      </div>
    </div>
  );
